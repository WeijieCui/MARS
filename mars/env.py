...
class SearchEnv:
    ...
    def set_all_target(self, target: str = ''):
    ...
    def reset(self):
        ...
        return (i, j, self.scale_idx, ACTIONS)
    ...
    def step(self, action: str) -> (Tuple)[
        ...
        if action == "up":
            ...
            if 'zoom_out' not in self.actions:
            ...
            if 'zoom_in' not in self.actions:
                self.actions.append('zoom_in')

        # 新状态
        i2, j2 = self._cell_of(self.cx, self.cy)
        status_new = (i2, j2, self.scale_idx, self.actions)
        return status_new, reward, obbs, crop
