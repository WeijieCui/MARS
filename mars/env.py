...
DEFAULT_SCALE_LEVELS = [0.008, 0.016, 0.032, 0.063, 0.125, 0.25, 0.5, 1.0]
...
class SearchEnv:
    def __init__(self, grid_n=10):
        ...
        self.scale_levels = None  # 缩放档位
        self.scale_idx = None  # 对应 ~0.7
    ...
    def set_image(self, image_bgr: np.ndarray):
        ...
        self.scale_idx = len(self.scale_levels) - 1  # 对应 ~0.7
        self.multi_grids = {
    ...
    def _cell_of_outter(self, x, y) -> Tuple[int, int]:
    ...
    def _cell_of_inner(self, x, y) -> Tuple[int, int]:
        """返回像素坐标所在的 10x10 单元索引 (i,j)"""
        if self.scale_idx <= 0:
            raise Exception
        grid_size = self._get_grid_size() * 2
    ...
    def step(self, action: str) -> (Tuple)[
        ...
        # 2) 状态转移：根据动作移动/缩放窗口
        step_px = max(20, self.scale_levels[self.scale_idx] * min(self.W, self.H))
        if action == "up":
    ...
    def refresh_actions(self):
        ...
        if 'zoom_out' in self.actions:
            if self.scale_idx < len(self.multi_grids) - 1:
                iu, ju = self._cell_of_outter(self.cx, self.cy)
        ...
        if 'zoom_in' in self.actions:
            if self.scale_idx > 0:
                iu, ju = self._cell_of_inner(self.cx, self.cy)
